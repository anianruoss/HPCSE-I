 ...
 */
template<typename T>
static T validate(const T *const M, const T *const truth, const int p,
                  const int q) {
    T res = 0.0;
    for (int i = 0; i < p; ++i)
        for (int j = 0; j < q; ++j) {
 ...
 */
template<typename T>
static void initialize(T *const A, T *const B, const int p, const int r,
                       const int q, const int seed = 101) {
...
void benchmark(const int p, const int r, const int q,
    ...
    auto tt2 = Clock::now();
    const double t =
            chrono::duration_cast<chrono::nanoseconds>(tt2 - tt1).count();
    ...
    cout << "  Number of elements: A=" << p * r << "; B=" << r * q
         << "; C=" << p * q << endl;
