 ...
 */
template <typename T>
T *read_gzfile_tile(const std::string &filename, size_t row_offset,
  ...
  auto *A = new T[tile_width * tile_height];
  auto *row = new T[tile_width];
  assert(A != nullptr);
  assert(row != nullptr);
 ...
 */
template <typename T>
void write_gzfile_tile(T *const data, const std::string &filename,
                       size_t tile_width, size_t tile_height) {
  int rank, size;
  ...
  if (isroot) {
    ...
    for (int i = 1; i < size; ++i) {
      MPI_Request req;
      MPI_Irecv(&buf[i * tile_size], tile_size,
                (sizeof(T) == 4) ? MPI_FLOAT : MPI_DOUBLE, i, 42,
                MPI_COMM_WORLD, &req);
      requests.push_back(req);
    }
    std::memcpy(buf, data, tile_size * sizeof(T));
  } else
    MPI_Send(data, tile_size, (sizeof(T) == 4) ? MPI_FLOAT : MPI_DOUBLE, 0, 42,
             MPI_COMM_WORLD);
  ...
  if (isroot) {
    ...
    if (fp == nullptr) {
      std::cout << "Gzip: Can not create file '" << filename << "'"
                << std::endl;
      std::exit(1);
    }
 ...
 * @param what Hint
 */
void report_time(Time &t1, Time &t2, const std::string &what) {
  int rank, size;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  const bool isroot = (0 == rank);
 ...
 */
template <typename T>
void show_stat(const T *const ref, const T *const test, const size_t N,
               const size_t myrows, const size_t compressed_bytes) {
  double lL1 = 0.;
  double lL2 = 0.;
  ...
  for (size_t i = 0; i < N * myrows; ++i) {
    ...
    const double diff =
        std::abs(static_cast<double>(test[i]) - static_cast<double>(ref[i]));
    lL1 += diff;
    lL2 += diff * diff;
  ...
  auto lcbytes = static_cast<double>(compressed_bytes);
  double cbytes = 0.;
  ...
  if (0 == rank) {
    ...
    std::cout << std::setw(32) << std::setfill('.') << std::left
              << "Uncompressed size"
              << ": " << std::scientific << ubytes / 1024. / 1024. << " MB"
              << std::endl;
    std::cout << std::setw(32) << std::setfill('.') << std::left
              << "Compressed size"
              << ": " << std::scientific << cbytes / 1024. / 1024. << " MB"
              << std::endl;
    std::cout << std::setw(32) << std::setfill('.') << std::left
              << "Compression rate"
              << ": " << std::scientific << compression_rate << '\n';
    std::cout << std::setw(32) << std::setfill('.') << std::left << "L1 error"
              << ": " << std::scientific << L1 << '\n';
    std::cout << std::setw(32) << std::setfill('.') << std::left << "L2 error"
              << ": " << std::scientific << L2 << '\n';
    std::cout << std::setw(32) << std::setfill('.') << std::left << "Linf error"
              << ": " << std::scientific << Linf << '\n';
    std::cout << std::setw(32) << std::setfill('.') << std::left << "PSNR"
              << ": " << std::scientific << psnr << " dB\n";
    std::cout << std::setw(32) << std::setfill('.') << std::left << "BITRATE"
              << ": " << std::scientific << bitrate << " bps" << std::endl;
  }
}
