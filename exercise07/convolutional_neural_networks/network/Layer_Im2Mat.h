...
struct Im2MatLayer : public Layer {
  ...
  void print() {
    printf("(%d) Im2Col transform Img:[%d %d %d] to Mat:[%d %d %d %d %d] ", ID,
           InY, InX, InC, OpY, OpX, KnY, KnX, InC);
  ...
  void Im2Mat(const int BS, const Real *const lin_inp,
              Real *const lin_out) const {
